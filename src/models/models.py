...
class GIN_adv(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        for i in range(len(self.convs)):
            ...
            x = F.relu(x)
            xs.append(x)
            self.node_embedding = x  # 保存节点嵌入
            # print(f"Shape after layer {i}: {x.shape}")
        
        x = torch.cat(xs, dim=1)
        # print(f"Shape after concatenation: {x.shape}")
        # print(f"Jump layer input size: {self.jump.in_features}, output size: {self.jump.out_features}")
        x = self.jump(x)
        return F.log_softmax(x, dim=-1)
...
def get_model(model_opt, nfeat, nclass, nhid=32, dropout=0, num_layers=2, eps=0, learn_eps=False, cuda=False):
       if model_opt == "GCN":
           model = GCN_Classifier(nfeat=nfeat, nhid=nhid, nclass=nclass, dropout=dropout)
       elif model_opt == "GCN_adv":
           model = GCN_adv(num_features=nfeat, num_classes=nclass)
       elif model_opt == "GIN":
           model = GIN(num_features=nfeat, num_classes=nclass)
       elif model_opt == "GIN_adv":
           ...
           model = GAT(num_features=nfeat, num_classes=nclass)
       else:
           raise ValueError(f"Unknown model option: {model_opt}")

       if cuda:
           model.cuda()
       print("Model:", model)
       return model
#usecase: model = get_model(args.model, self.features.size(1), 2, args.hidden, args.dropout,
                        #   args.cuda)
